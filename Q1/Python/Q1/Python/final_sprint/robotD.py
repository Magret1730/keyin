...
def detect_marker():
    ...
    marker_id = marker_info[1]  # assuming 1 is ID position
...
def handle_marker(marker, position):
    if marker == "F":
        print(f"Fire at {position} — Shooting!")
        gimbal_ctrl.pitch_ctrl(10)
        time.sleep(1)
        blaster_ctrl.fire()
        
    elif marker == "D":
        print(f"Dangerous zone at {position} — Skipping.")

    elif marker == "P":
        print(f"Person at {position} — Returning to A.")
        go_back_to_A()
        return_to_position(position)

    else:
        print(f"No valid marker detected at {position}.")


def go_back_to_A():
    chassis_ctrl.move_with_distance(180, 3.0)  # Adjust distance as needed
    time.sleep(1)
...
def detect_marker():
    ...
    if marker_info[1] == 1:
        return "F"  # Fire
    elif marker_info[1] == 2:
        return "D"  # Dangerous
    elif marker_info[1] == 3:
        return "P"  # Person
    else:
        return "Unknown"
...
def handle_marker(marker):
    if marker == "F":
        print("Fire detected! Shooting now.")
        gimbal_ctrl.pitch_ctrl(10)
        time.sleep(1)
        blaster_ctrl.fire()
        
    elif marker == "D":
        print("Danger zone detected. Skipping this post.")

    elif marker == "P":
        print("Person detected! Returning to safety.")
        go_back_to_A()
        return_to_post()
...
def go_back_to_A():
    chassis_ctrl.move_with_distance(180, 3.0)  # Adjust to return

def return_to_post():
    chassis_ctrl.move_with_distance(0, 3.0)  # Adjust to resume
